...
///////////////////////////////////////////////////////////////////////////////
bullet_t *create_bullet(actor_t *sender, v2f_t direction, weapon_enum_t weapon)
{
...
weapon_enum_t search_weapon(actor_t *act)
{
    for (int i = 0; i < Entities.count; i++) {
        if (act == Entities.array[i]->actor && act != Player.ref)
            return (Stats[act->self->id].hideWeapon ? -1 :
                Entities.array[i]->weapon);
    }
    return (-1);
}
